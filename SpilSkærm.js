class SpilSkærm {
  ...
  run() {
    ...
    if (passedTime >= 1000) {
      if (this.spiller1True === true && this.tid1 > 0)
        this.tid1 -= 1;
      else if (this.spiller1True === true && this.tid1 == 0) {
         loser = 'white'
        manager.opretGameState('slut', new SlutSkærm());
        manager.skiftGameState('slut')
      }
      if(this.spiller1True === false && this.tid2 > 0)
        this.tid2 -= 1;
      else if (this.spiller1True === false && this.tid2 == 0){
        loser = 'black'
        manager.opretGameState('slut', new SlutSkærm());
        manager.skiftGameState('slut')
      }
    ...
    } else if (this.spiller1True === false){
      fill(0);
        text("HVID: " + this.tid1, width/2, height/2.5);
      fill(255)
      text("SORT: " + this.tid2, width/2, height/1.6);
